...
// Mengambil pesanan yang statusnya BUKAN 'active' (artinya sudah di-checkout)
$stmt = $conn->prepare("
    SELECT 
        c.id, 
        c.order_status, 
                ...
                <?php foreach ($orders as $order): ?>
                    <div class="bg-white rounded-lg shadow-md p-4 transition hover:shadow-lg">
                        <div class="flex justify-between items-start">
                            ...
                            <a href="detail_order.php?order_id=<?= $order['id'] ?>" class="bg-pink-500 text-white text-sm font-bold py-2 px-4 rounded-full hover:bg-pink-600 transition-colors">
                                Details
                            </a>
                        </div>
                    </div>
