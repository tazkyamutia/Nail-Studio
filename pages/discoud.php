...
$countSql = "SELECT COUNT(*) AS total FROM product WHERE (status = 'published' OR status = 'low stock') AND stock > 0 AND discount > 0";
  ...
  <div class="max-w-5xl mx-auto px-4 rounded-lg bg-[#d7e6fb]">
    <div class="inline-block mb-6">
      <nav class="flex items-center space-x-2 text-gray-700 text-sm bg-white rounded-full px-3 py-1 select-none">
        ...
        <span class="text-gray-900 text-xs">Diskon</span>
      </nav>
    </div>
    <div class="max-w-5xl mx-auto pr-6 pl-2 py-4 bg-[#d7e6fb] rounded-lg">
      <h1 class="text-3xl font-semibold text-gray-900 mb-3 text-left pl-0">Discover Incredible Deals with Discounted Products!</h1>
      <p class="text-gray-900 mb-3 text-base max-w-full text-left pl-0">
       Get ready to indulge in unbeatable savings! Our collection of discounted products is your gateway to fantastic offers and price cuts that you won’t want to miss. Whether you're looking for beauty essentials, health care, or household items, we’ve got something for everyone, all at incredible discounted prices.

From nourishing nail care items to the latest beauty tools, you can enjoy a wide range of high-quality products at a fraction of the original price. It's the perfect opportunity to stock up on your favorites or try something new, all while saving big.

Don’t wait—our discounts won’t last forever! Shop now and enjoy premium products without breaking the bank. Embrace great savings while treating yourself to something special.
      </p>
    </div>
  </div>
</div>
...
<div class="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-6">
    <?php foreach ($products as $product): ?>
        ...
        if ($productDiscount > 0) {
            ...
            $discountMessage = "<span class='text-green-500'>Harga Normal</span>";
        }
