...
$stmt_items = $conn->prepare("SELECT SUM(qty * price) as total_price FROM cart_item WHERE cart_id = ?");
$stmt_items->execute([$order_id]);
$total_price = $stmt_items->fetchColumn();

// 4. Logika untuk Checklist Status
...
// === PERUBAHAN DI SINI ===
// Placeholder untuk Metode Pembayaran diubah menjadi QRIS
$payment_method = "QRIS";
?>
    ...
    <div class="container mx-auto max-w-4xl py-20 px-4">
        <div class="bg-white rounded-xl shadow-lg p-6 md:p-8">
            ...
            <div class="mb-8">
                ...
                <div class="flex items-center justify-between w-full">
                    <div class="flex-1 text-center">
                        <div class="mx-auto h-12 w-12 rounded-full flex items-center justify-center <?= $isPacked ? 'bg-green-500' : 'bg-gray-300' ?>">
                            <i class="fas fa-box-open text-white text-xl"></i>
                        </div>
                        <p class="mt-2 text-sm font-medium <?= $isPacked ? 'text-green-600' : 'text-gray-500' ?>">Being packed</p>
                    </div>

                    <div class="flex-1 h-1 <?= $isInProgress ? 'bg-green-500' : 'bg-gray-300' ?>"></div>

                    <div class="flex-1 text-center">
                        <div class="mx-auto h-12 w-12 rounded-full flex items-center justify-center <?= $isInProgress ? 'bg-green-500' : 'bg-gray-300' ?>">
                            <i class="fas fa-truck text-white text-xl"></i>
                        </div>
                        <p class="mt-2 text-sm font-medium <?= $isInProgress ? 'text-green-600' : 'text-gray-500' ?>">In Progress</p>
                    </div>

                    <div class="flex-1 h-1 <?= $isComplete ? 'bg-green-500' : 'bg-gray-300' ?>"></div>

                    <div class="flex-1 text-center">
                        <div class="mx-auto h-12 w-12 rounded-full flex items-center justify-center <?= $isComplete ? 'bg-green-500' : 'bg-gray-300' ?>">
                            <i class="fas fa-check-circle text-white text-xl"></i>
                        </div>
                        <p class="mt-2 text-sm font-medium <?= $isComplete ? 'text-green-600' : 'text-gray-500' ?>">Complete</p>
                    </div>
                </div>
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mt-8 pt-6 border-t">
                <a href="index.php" class="w-full md:w-auto text-center py-2 px-5 mb-2 md:mb-0 rounded-full bg-gray-200 text-gray-700 hover:bg-gray-300 transition">
                    Continue Shopping
                </a>
                <button onClick="window.location.reload()" class="w-full md:w-auto text-center py-2 px-5 rounded-full bg-pink-500 text-white font-semibold hover:bg-pink-600 transition">
                    Refresh Status
                </button>
            </div>
